...
<%
	if(session.getAttribute("userList") == null){
		request.getRequestDispatcher("/user.s?method=findUserList").forward(request, response);
	}
%> 
<%-- <c:if test="${empty userList }">
	<jsp:forward page="/user.s?method=findUserList"></jsp:forward>
</c:if> --%>
...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <h1 class="page-header">操作</h1>
        ...
        <h1 class="page-header">管理 <span class="badge">${IndexShowBeen.adminCount }</span></h1>
        <form action="user.s">
        	<%-- <input name="method" type="hidden" value="query">
          ...
          <table class="table table-striped table-hover">
            ...
            <c:forEach items="${userList }" var="user">
	            <tr>
					...
					<td><a rel='${user.id }' name='see'>修改</a> <a rel='${user.id }' name='delete'>删除</a> <a href='/User/checked/id/1/action/n'>禁用</a></td>
				</tr>
            </c:forEach>
    ...
    <form action="/User/update" method="post" autocomplete="off" draggable="false">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
          <button type="button" class="btn btn-primary" onclick="save()"><a href="" id="1">提交</a></button>
        </div>
      </div>
    </form>
  </div>
